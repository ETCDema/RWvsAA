...
1. Использовать ViewBag для передачи нужных для генерации HTML
...
1. Раскладка данных и валидация происходят до выполнения действия контроллера и никак не контролируется
...
- Роутинг, который в итоге и приведет нас к вызову метода контроллера и который тоже является одим из настроенных Middleware
- Механизм, опеределяющий нужный контроллер и инициализирующий его
...
1. Проверить права пользователя и доступность выполнения действия в текущем состоянии данных
1. Определить правила и ограничения работы с данными
